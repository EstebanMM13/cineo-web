  ...
  template: `
    <div>
      <div class="flex justify-between items-center mb-6">
        ...
        <button (click)="openCreate()" class="px-4 py-2 rounded text-sm font-medium" style="background:var(--color-cinema-gold);color:var(--color-cinema-bg);">
          + Nueva película
        </button>
      </div>
      ...
      <div *ngIf="showForm" class="mb-6 p-5 rounded-lg" style="background:#1a1a1a;border:1px solid #2a2a2a;">
        ...
        <div class="flex gap-2 mt-4">
          <button (click)="save()" [disabled]="saving" class="px-4 py-2 rounded text-sm font-medium disabled:opacity-40" style="background:var(--color-cinema-gold);color:var(--color-cinema-bg);">
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr *ngFor="let m of movies" class="transition-colors hover:bg-[#1f1f1f]" style="border-top:1px solid #2a2a2a;">
              ...
              <td class="px-4 py-3" style="color:var(--color-cinema-gold);">&#9733; {{ m.rating | number:'1.1-1' }}</td>
