    ...
    <div class="max-w-lg">
      ...
      <div class="flex gap-2 mb-6">
        <input
          ...
          (keyup.enter)="create()"
        />
        <button (click)="create()" [disabled]="!newName.trim() || saving" class="px-4 py-2 rounded text-sm font-medium disabled:opacity-40" style="background:#e50914;color:#fff;">
          Añadir
        </button>
      </div>
      ...
      <div class="flex flex-col gap-2">
        <div *ngFor="let g of genres" class="flex items-center gap-3 px-4 py-3 rounded-lg" style="background:#1a1a1a;border:1px solid #2a2a2a;">
          ...
          <div class="flex gap-2">
            ...
            <button *ngIf="editingId === g.id" (click)="saveEdit(g)" class="text-xs px-2 py-1 rounded" style="background:#e50914;color:#fff;">Guardar</button>
