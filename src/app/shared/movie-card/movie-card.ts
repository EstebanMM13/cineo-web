...
@Component({
  ...
  styles: [`
    .card { display:block; text-decoration:none; background:var(--color-cinema-surface); border:1px solid #1a1a1a; border-radius:12px; overflow:hidden; transition:transform 0.25s ease,box-shadow 0.25s ease,border-color 0.25s ease; cursor:pointer; }
    ...
    .overlay-btn { background:var(--color-cinema-gold); color:var(--color-cinema-bg); font-size:11px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; padding:8px 16px; border-radius:8px; }
    ...
    <a class="card" [routerLink]="['/movies', movie.id]">
      <div style="position:relative;aspect-ratio:2/3;background:#111;overflow:hidden;">
        ...
        <ng-template #placeholder>
          <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:var(--color-cinema-border);">
        ...
        <div style="position:absolute;top:8px;right:8px;display:flex;align-items:center;gap:4px;padding:4px 7px;border-radius:7px;background:rgba(0,0,0,0.8);backdrop-filter:blur(6px);">
          <svg width="9" height="9" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="1" style="color:var(--color-cinema-gold);"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
          <span style="font-size:11px;font-weight:700;color:var(--color-cinema-gold);">{{ movie.rating | number:'1.1-1' }}</span>
        </div>
      ...
      <div style="padding:12px;">
        <h3 style="font-size:13px;font-weight:600;color:var(--color-cinema-text);margin:0 0 3px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ movie.title }}</h3>
